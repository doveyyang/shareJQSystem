	...
	<view class="detail-container">
		...
		<view class="project-box flex flex-direction justify-center">
			...
			<view class="address-box padding-sm">
				...
				<text class="address text-blue">{{address}}</text>
			</view>
			
			<view class="status-box">
				<image src="/static/img/sea/yiwancheng.png" mode="widthFix"></image>
			</view>
		</view>
		
		<view class="picture-box margin-top">
			<view class="title text-center text-lg text-blue padding-sm">
				- 现场实拍 -
			</view>
			<view class="">
		...
		<view class="desc-box margin-top">
			<view class="title text-center text-lg text-blue padding-sm">
				- 净入寄语 -
			</view>
			<view class="content padding-sm">				
				空气污染是很多人关心的话题，人们在茶余饭后都会讨论，不过也不用觉得甲醛就是恐怖的存在。只要从源头上控制，装修后期检测甲醛和除甲醛，进行车内室内空气治理很重要。
				
			</view>
			<view class="content padding-sm">
				每次治理员做项目时，都会对其讲解甲醛基本知识，告知其实最可怕的不是甲醛污染，而是大家意识不到或意识到了却用错误的方法去应对，有新房的朋友建议给新房“体检”，入住前找权威机构做一个环保检测，费用不高，甲醛这类污染物是无色无味的，做个检测才放心入住。在此希望大家想做除醛时，请与专业除甲醛公司联系！还您舒适安全的环境！
			</view>
		</view>
		<view class="logo-box padding-lg margin-top">
			<view class="logo">
				<image src="/static/img/sea/logo.png" mode=""></image>
			</view>
			<view class="slogan">
				净入让洁净的空气净入千家万户
			</view>
	...
	export default {
		...
		methods:{
			initData(){
				...
				uni.request({
					url:service.BASEURL+'/home/share/case_anfos',
					method:"POST",
					data:{
						id:this.id
					},
					success(res) {
						...
						if(!res.data || res.data.code != 1){
							...
							if(!res.data || res.data.code !=1){
								...
								self.area = info.area;						
								if(info.img){
									self.imgList = info.img.map(e=>(service.BASEURL+e.url));
								}
